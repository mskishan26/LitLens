...
def setup_logger(name: str, log_type: str = 'general', log_level=logging.INFO, max_files: int = None):
    ...
    # Allow overriding console log level via env var
    console_level_name = os.getenv('RAG_CONSOLE_LOG_LEVEL')
    if console_level_name:
        console_handler.setLevel(getattr(logging, console_level_name.upper(), log_level))
    else:
