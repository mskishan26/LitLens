...
class Reranker:
    ...
    def __init__(
        ...
        batch_size: int = 4,
        max_length: int = 8192,
        max_ram_percent: float = 85.0,  # Exit if RAM usage exceeds this
        check_memory_every_n_batches: int = 1  # Check memory every N batches
    ):
        """
        ...
        Args:
            ...
            max_ram_percent: Maximum RAM usage percentage before exiting (default: 85%)
            check_memory_every_n_batches: Check memory every N batches (default: 1)
        """
        ...
        self.check_memory_every_n_batches = check_memory_every_n_batches
        
        # Get initial memory stats
        self._log_memory_stats("Initial memory")
        ...
        self._log_memory_stats("After model load")
    ...
    def _compute_scores(self, query: str, texts: List[str]) -> np.ndarray:
        ...
        # Process in batches
        for batch_idx, i in enumerate(tqdm(range(0, len(texts), self.batch_size), desc="Reranking")):
            # Check memory before processing batch
            if batch_idx % self.check_memory_every_n_batches == 0:
                self._check_memory_and_exit_if_needed(f"Before batch {batch_idx}")
    ...
    def _get_memory_stats(self) -> Dict[str, float]:
        ...
        Returns:
            Dict with RAM and GPU memory stats
        """
        stats = {}
    ...
    def _log_memory_stats(self, context: str = ""):
        ...
        msg = f"[{context}] RAM: {stats['ram_used_gb']:.2f}GB / {stats['ram_total_gb']:.2f}GB ({stats['ram_percent']:.1f}%)"
        
        if 'gpu_allocated_gb' in stats:
            msg += f" | GPU: {stats['gpu_allocated_gb']:.2f}GB / {stats['gpu_total_gb']:.2f}GB"
        
        logger.info(msg)
    
    def _check_memory_and_exit_if_needed(self, context: str = ""):
        """
        Check current memory usage and exit if it exceeds threshold.
        
        Args:
            context: Context string for logging
        
        Raises:
            MemoryError: If RAM usage exceeds max_ram_percent
        """
        stats = self._get_memory_stats()
        
        if stats['ram_percent'] > self.max_ram_percent:
            self._log_memory_stats(context)
            error_msg = (
                f"RAM usage ({stats['ram_percent']:.1f}%) exceeded maximum threshold "
                f"({self.max_ram_percent}%). Stopping to prevent crash.\n"
                f"Current usage: {stats['ram_used_gb']:.2f}GB / {stats['ram_total_gb']:.2f}GB\n"
                f"Consider: (1) reducing batch_size, (2) reducing max_length, "
                f"(3) using fewer candidates, or (4) using a smaller model"
            )
